...
class CartPoleAgent:
    ...
    def update_sarsa(self, state, action, reward, new_state, new_action):
        self.sarsa_table[state][action] += self.learning_rate * (
            reward
            + self.discount * (self.sarsa_table[new_state][new_action])
            - self.sarsa_table[state][action]
        )

    def get_epsilon(self, t):
        return max(self.min_epsilon, min(1.0, 1.0 - math.log10((t + 1) / self.decay)))

    def get_learning_rate(self, t):
        return max(self.min_lr, min(1.0, 1.0 - math.log10((t + 1) / self.decay)))
    ...
    def train(self):
        ...
        for e in range(self.num_episodes):
            ...
            print(
                f"episode: {e}, reward: {round(reward_sum*0.01 + best_reward*0.99, 2)}"
            )
